...
export class RoomHandoffs {
  ...
  enqueue(source: RoomAddress, generation: string, parentId: string | undefined,
    ...
    // prior result within the same source bot/room and task revision. A key
    // must not expose another caller's results or cross a revoked route.
    // Explicit rework permits a concrete correction or repeated check.
    if (stateKey && !rework) {
      const prior = [...this.nodes.values()].reverse().find(n => {
        const author = n.parentId ? this.nodes.get(n.parentId) : undefined;
        return n.stateKey === stateKey && n.kind === kind &&
          n.groupId === target.groupId && n.botId === target.botId &&
          n.workItemId === work?.workItemId && n.workRevision === work?.workRevision &&
          author?.botId === source.botId && author.groupId === source.groupId;
      });
      if (prior) {
        const problem = this.hooks.validate(prior, this.nodes.get(prior.parentId!))
          ?? this.hooks.validate({ ...prior, approvalGranted }, parent);
        if (problem) throw new Error(problem);
        return { node: prior, duplicate: true };
      }
    }
