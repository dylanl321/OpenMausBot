...
describe("additive portable team backups", () => {
  it.each([false, true])("remaps shared task hubs and worker links without replaying imported execution (owner-local keys: %s)", ownerLocalKeys => {
    ...
    if (ownerLocalKeys) {
      for (const owner of [...backup.bots, ...backup.groups]) {
        owner.tasks.find(task => task.key === owner.activeTask)!.key = "main";
        owner.activeTask = "main";
      }
      backup.workItems![0].threadId = "main";
      backup.workItems![0].assignments[0].threadId = "main";
    }
    const imported = importTeamBackup(store, routines, backup, selection(), { workItems: items });
