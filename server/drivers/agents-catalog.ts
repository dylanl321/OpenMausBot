...
const toolDefinitions = (externalRuntime: boolean) => [
  ...
  {
    name: "ensure_work_item",
    description: "Before assigning a deliverable or ongoing multi-bot work, create or reuse its shared task. This automatically creates/reuses a topic room and task hub, then schedules its coordinator there. Use a stable identity including source/account and item ID (not a title or timestamp); for generic work choose a durable task key and keep reusing it. Use input only for relevant changed requirements/source content, never polling timestamps, acknowledgements or your own status comments. Unchanged active/completed/blocked work is reused without another execution. No files or permissions are transferred. End the source turn after acceptance; the task's final outcome returns automatically. Quick standalone advice does not need a task.",
    inputSchema: { type: "object", additionalProperties: false, properties: {
      work_item_id: { type: "string" }, group_id: { type: "string" }, topic: { type: "string", maxLength: 100 },
      identity: { type: "string", maxLength: 240 }, title: { type: "string", maxLength: 80 }, objective: { type: "string", maxLength: 4000 },
      acceptance_criteria: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 20 },
      input: { type: "string", maxLength: 12000 },
    }, required: ["title", "objective", "acceptance_criteria"] },
  },
  {
    name: "get_work_item",
    description: "Read a shared task's current brief, revision, assignments, blockers and recorded results. Omit work_item_id for the current task and accessible task directory. Read on a new instruction or returned result; do not poll waiting teammates.",
    inputSchema: { type: "object", additionalProperties: false, properties: { work_item_id: { type: "string" } } },
  },
  {
    name: "update_work_item",
    description: "Coordinator only, from the shared hub: record a decision, artifacts or terminal outcome against expected_revision. Completion needs a concrete detail, actual evidence references and every acceptance criterion in completed_criteria; all current assignments must have succeeded. Use blocked/needs-input for missing evidence or authority. Do not change requirements, reopen unchanged work or claim tests ran without execution evidence.",
    inputSchema: { type: "object", additionalProperties: false, properties: {
      work_item_id: { type: "string" }, expected_revision: { type: "integer", minimum: 1 },
  ...
  },
  {
    ...
    description: "Contact actual OpenMausBot teammates using their own models and permissions. For deliverables use intent=work and ensure_work_item first: the shared hub owns coordination, and specialists execute in task-specific linked threads. A task-bound turn inherits its task automatically. Only quick standalone advice uses intent=consultation, retaining the existing room or standing bot-pair conversation. Give 1-4 exact bot_ids, a self-contained brief, accessible artifact paths and acceptance checks. End after assigning; busy bots queue and results resume you automatically. Do not poll, send acknowledgements as assignments, invent new keys for unchanged work, or substitute native helper agents. Concrete rework requires the returned assignment_id, rework=true and a changed correction brief.",
    inputSchema: { type: "object", additionalProperties: false, properties: {
      ...
      intent: { type: "string", enum: ["work", "consultation"], description: "Use work for deliverables or ongoing assignments; consultation only for quick standalone advice. Omission retains compatibility with older consultation callers." },
      work_item_id: { type: "string", description: "Shared task ID; inherited inside its hub and worker threads." },
      assignment_id: { type: "string", description: "Existing assignment ID returned by this tool, required for concrete rework." },
      ...
      request_key: { type: "string", description: "A short key unique within this turn. Reuse for an identical retry." },
      state_key: { type: "string", description: "Optional stable delivery-state key, such as project:ticket:artifact-revision:phase:gate:owner. Reuse it across threads and routine runs while that state is unchanged; a prior status/result is returned without starting duplicate work." },
    ...
    }, required: ["bot_ids", "message", "request_key", "intent"] },
  },
  {
    name: "list_bots",
    description:
