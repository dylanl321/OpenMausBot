...
describe("OpenAICompatDriver", () => {
  ...
  it("exposes configured Bedrock models in the custom picker with readable labels", async () => {
    ...
    expect(inst.models).toEqual({
      ...
      options: [
        { id: "us.openai.gpt-6-sol", label: "GPT-6 Sol · us-east-1", custom: true },
        { id: "us.openai.gpt-6-luna", label: "GPT-6 Luna · us-east-1", custom: true },
        { id: "vendor.other-model", label: "vendor.other-model · us-east-1", custom: true },
      ],
    });
    await inst.dispose();
  });
  ...
  it("routes GPT-6 models to their configured region and uses supported reasoning", async () => {
    ...
    const recorder = recordEvents(inst.adapter);
    expect(inst.models.options).toEqual([{ id: "us.openai.gpt-6-luna", label: "GPT-6 Luna · us-east-2", custom: true }]);
  ...
  it("retains models with explicit regional routes when the default endpoint has no model catalog", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => new Response("Not found", { status: 404 })));
    const inst = await OpenAICompatDriver.create({ instanceId: "regional-models", displayName: "Regional models", enabled: true,
      config: { url: "https://bedrock-runtime.us-east-1.amazonaws.com/openai/v1", apiKeyEnv: "TEST_KEY",
        model: "us.openai.gpt-6-sol", modelUrls: { "us.openai.gpt-6-luna": "https://bedrock-runtime.us-east-2.amazonaws.com/openai/v1" } },
      environment: { TEST_KEY: "synthetic-token" } });
    await inst.refreshModels!();
    expect(inst.models).toEqual({ default: "us.openai.gpt-6-sol", options: [
      { id: "us.openai.gpt-6-sol", label: "GPT-6 Sol · us-east-1", custom: true },
      { id: "us.openai.gpt-6-luna", label: "GPT-6 Luna · us-east-2", custom: true },
    ] });
    await inst.dispose();
  });
