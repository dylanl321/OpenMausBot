...
export class WorkItems {
  ...
  constructor(file: string, now: () => number = Date.now) {
    ...
    this.now = now;
    try {
      const saved = z.array(recordSchema).max(10_000).parse(JSON.parse(readFileSync(file, "utf8")));
  ...
  update(record: WorkRecord, input: WorkUpdate, actorBotId?: string) {
    ...
    if (input.reopen || input.objective !== undefined || input.acceptanceCriteria !== undefined) {
      ...
      if (input.acceptanceCriteria) item.acceptanceCriteria = input.acceptanceCriteria;
      ...
      if (input.status === "completed") {
        ...
        if (!input.evidence?.length || item.acceptanceCriteria.some(criterion => !input.completedCriteria?.includes(criterion))) {
    ...
    if (input.artifacts) item.artifacts = input.artifacts;
