...
export function importTeamBackup(store: Store, routines: RoutineManager, input: unknown, selection: ModelSelection, workItems?: WorkItems) {
  ...
  const restore = (task: BackupTask, threadId: string) => {
    threadIds.set(task.key, threadId);
  ...
  try {
    ...
    for (const source of workItems ? backup.workItems ?? [] : []) {
      ...
      const threadId = threadIds.get(source.threadId)!;
      ...
      const item = workItems!.restore({ ...source, id, groupId, threadId, coordinatorBotId: botIds.get(source.coordinatorBotId)!,
        scope: store.group(groupId)!.section ?? "", assignments: source.assignments.map(assignment => ({ ...assignment,
          id: newId(), botId: botIds.get(assignment.botId)!, threadId: threadIds.get(assignment.threadId)!, requestId: undefined })) });
