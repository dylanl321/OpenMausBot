...
describe("scopes", () => {
  ...
  it("is default deny: chat, approvals, rooms, attachments, routines and own session are client; everything else admin", () => {
    for (const [method, path] of [
      ...
      ["GET", "/api/work-items/w/events"],
      ["POST", "/api/work-items/w/links"],
      ["GET", "/api/task-connectors"],
      ...
      ["GET", "/api/config"], ["GET", "/api/webhooks"], ["GET", "/api/watches"], ["POST", "/api/tts/speak"],
    ...
    for (const [method, path] of [
      ...
      ["POST", "/api/webhooks"], ["POST", "/api/webhooks/w/rotate"], ["POST", "/api/watches"], ["POST", "/api/watches/dry-run"], ["PATCH", "/api/watches/w"],
      ["POST", "/api/bots/x/skills"], ["PATCH", "/api/bots/x/skills/s"],
