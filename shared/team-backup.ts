...
export function parseTeamBackup(input: unknown): TeamBackup {
  ...
  for (const item of backup.workItems ?? []) {
    ...
    const hubKey = JSON.stringify([item.groupId, item.threadId]);
    if (sharedThreads.has(hubKey)) throw new Error("Invalid backup: shared task hub has multiple owners");
    sharedThreads.add(hubKey);
