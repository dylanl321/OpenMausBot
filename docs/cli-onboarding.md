...
2. Sign in with the provider, or paste an API key into the hidden prompt. Setup asks before installing a missing Codex or Claude CLI. Codex also offers device-code sign-in for a remote terminal. API choices include OpenAI, OpenRouter, Groq, other OpenAI-compatible endpoints, and [Amazon Bedrock](bedrock.md) with a token, AWS profile or access keys.
3. Choose a model and save. OpenAI-compatible connections ask permission to send a short test message, which the provider may charge for. Bedrock reads its regional model catalog and offers US-only and Anthropic access settings without invoking a model. Native CLI setup confirms sign-in without sending a message. Model access is checked when you send one.
...
Codex and Claude Code support agent tools. Bedrock and OpenAI-compatible engines also support approved MCP tools when the selected model supports tool calling. A ChatGPT or Claude subscription does not include separately billed API usage. Changing the saved default affects **new bots only**; existing bots and conversations keep their settings.
