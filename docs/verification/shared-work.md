...
```sh
pnpm exec vitest run server/work-items.test.ts server/shared-work.e2e.test.ts server/team-backup.test.ts server/workspace-backup.test.ts server/room-handoffs.test.ts --maxWorkers=2
```
...
It retains `.shared-task.png` and `.shared-task.json` beside the fixture's
printed server log, plus `.topic-worker.png` showing the selected worker in
its topic tree. The JSON includes the task record and selected thread.
